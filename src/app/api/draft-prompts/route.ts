...
// Phase 3c item 5: returns all 4 draft prompts (base + 3 author voices) so
// the editor can render every row in one shot. chars is computed
...
export async function GET() {
  try {
    ...
    if (error) {
      console.error('GET draft-prompts query failed:', error)
