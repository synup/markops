...
export function useDraftPrompts(): Result {
  const [prompts, setPrompts] = useState<DraftPromptRow[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const resp = await fetch('/api/draft-prompts', { signal: ctrl.signal, cache: 'no-store' })
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`)
        const data = (await resp.json()) as { prompts: DraftPromptRow[] }
        if (cancelled) return
  ...
  const saveSelected = useCallback(async () => {
    ...
    try {
      const resp = await fetch(
        `/api/draft-prompts/${encodeURIComponent(selectedPromptName)}`,
        {
          method: 'PATCH',
      ...
      const data = (await resp.json()) as { prompt: DraftPromptRow }
