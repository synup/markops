#!/usr/bin/env python3
"""
Generate content drafts for approved call_insights.
Phase 3c of the Content Intelligence Workflow.

Prompts loaded from content_draft_prompts table (Supabase) as of migration 020.
Disk files in draft_prompts/ retained as seed-source reference only — no longer
read at runtime. Edits to the table (via SQL today, via UI after Phase 3c item 5)
take effect on the next cron run since prompts are cached only per-invocation.

For each content_drafts row with status='pending' (oldest first):
  ...
  3. Compose system prompt = base + <author_voice> rows from content_draft_prompts.
  ...
  5. PATCH draft_content, prompt_used, model_version, generation_metadata,
...
Retry policy (inline within a single invocation — no cross-run backoff):
  ...
  After attempt 3 fails: status='failed' (terminal), move to next draft.

Worst case ~90s of inline wait + 3x Claude call latency per failing draft.
That cost is bounded per draft so a bad row cannot freeze the queue or cause
overlapping cron runs.

Per-run caps:
  - At most MAX_PER_RUN (=10) candidates fetched.
  - Drafts processed sequentially (concurrency=1).

Usage:
  python process_content_drafts.py [--dry-run] [--max-drafts N] [--draft-id UUID]
...
# Worst case ~90s of inline wait per failing draft.
...
VALID_AUTHOR_VOICES = ('sudy', 'roshan', 'niladri')
VALID_PROMPT_NAMES = ('base',) + VALID_AUTHOR_VOICES
...
class FetchError(Exception):
    """Per-draft transient fetch failure (joined call_insight / sales_call,
    or transient prompt fetch) — inline-retried up to MAX_ATTEMPTS."""


class PromptNotFoundError(Exception):
    """Requested prompt row missing from content_draft_prompts — config
    issue, not transient. Treated as TERMINAL failure for the draft."""


class GenerationError(Exception):
    """Per-draft failure in Claude call / parse / DB write — skip this draft."""
...
def parse_args():
    p = argparse.ArgumentParser(
        description='Generate content drafts for approved call_insights.',
    )
    p.add_argument(
        '--dry-run', action='store_true',
        help='Build the prompt and call Claude (costs apply), but do not '
             'claim the row or write the result. Prints the generated draft '
             'to stdout.',
    )
    p.add_argument(
        '--max-drafts', type=int, default=None, metavar='N',
        help='Process at most N drafts this run.',
    )
    p.add_argument(
        '--draft-id', type=str, default=None, metavar='UUID',
        help='Process a single specific draft (ignores status filter; useful '
...
# (matches cron cadence) plus the in-flight run's remaining drafts.
_prompt_cache: dict = {}


def get_prompt(name: str) -> str:
    """Fetch a prompt row's content from content_draft_prompts, with cache.

    Raises:
      PromptNotFoundError  row missing for `name` — TERMINAL for the draft.
    ...
    url = (
        f'{SUPABASE_URL}/rest/v1/content_draft_prompts'
    ...
    if not rows:
        raise PromptNotFoundError(
            f"Prompt {name!r} missing from content_draft_prompts table — check seed"
        )
...
# ─── Supabase reads ───────────────────────────────────────────────────
def fetch_pending_drafts(limit, draft_id=None):
    """GET content_drafts — pending rows oldest first (or a specific id)."""
    params = [
        ('select', 'id,call_insight_id,author_voice,status,retry_count,created_at'),
        ('limit',  str(limit)),
    ]
    if draft_id:
        params.append(('id', f'eq.{draft_id}'))
    else:
        params.append(('status', 'eq.pending'))
        params.append(('order',  'created_at.asc'))

    url = f'{SUPABASE_URL}/rest/v1/content_drafts?{urlencode(params)}'
    req = Request(url, headers=_supabase_headers())
    try:
        with urlopen(req, timeout=HTTP_TIMEOUT_S) as resp:
        ...
        raise FetchError(
            f'fetch_drafts_failed http_status={e.code} detail={_read_body(e)[:300]!r}'
        ) from e
    except URLError as e:
        raise FetchError(f'fetch_drafts_url_error={e.reason!r}') from e


def fetch_call_context(call_insight_id):
    """GET call_insights row + embedded sales_calls (the source conversation)."""
    # Drafts pull the same source-conversation context briefs do, minus
    # approved_asset_type / suggested_asset_type (the row's author_voice is
    # authoritative for drafts, so those call_insights columns are unused).
    select = (
        'id,call_id,marketing_summary,customer_verbatim,problem_statement,'
        'asset_rationale,suggested_author,'
        'composite_score,icp_fit_score,problem_clarity_score,'
...
def claim_draft(draft_id):
    ...
    url = (
        f'{SUPABASE_URL}/rest/v1/content_drafts'
        f'?id=eq.{quote(draft_id, safe="")}'
        f'&status=eq.pending'
    )
...
def write_success(draft_id, draft_content, prompt_used, metadata):
    """PATCH draft row with successful generation output."""
    payload = {
        'status':              'ready',
    ...
    _patch_draft(draft_id, payload, 'write_success_failed')


def write_terminal_failure(draft_id, error_message):
    """PATCH draft row with status='failed' after MAX_ATTEMPTS exhausted."""
    payload = {
        'status':        'failed',
        'error_message': error_message[:1000],
        'retry_count':   MAX_ATTEMPTS,
    }
    _patch_draft(draft_id, payload, 'write_failure_failed')


def write_attempt_progress(draft_id, attempt, error_message):
    ...
    _patch_draft(draft_id, payload, 'write_attempt_progress_failed')


def release_draft(draft_id, error_message):
    ...
    _patch_draft(draft_id, payload, 'release_draft_failed')


def _patch_draft(draft_id, payload, error_event):
    url = (
        f'{SUPABASE_URL}/rest/v1/content_drafts'
        f'?id=eq.{quote(draft_id, safe="")}'
    )
    ...
    try:
        ...
        log('error', error_event, draft_id=draft_id, detail=repr(e))


# ─── Anthropic call ───────────────────────────────────────────────────
def build_system_prompt(author_voice):
    """Concatenate the 'base' prompt + the author-voice-specific prompt from
    content_draft_prompts. Propagates PromptNotFoundError (terminal) and
    FetchError (transient) to the caller for handling."""
    base  = get_prompt('base')
    voice = get_prompt(author_voice)
    return f'{base}\n\n---\n\n{voice}'


def build_user_message(context, author_voice):
    """Render call_insight + sales_call as source material for the draft.
    The row's author_voice (sudy / roshan / niladri) is passed in
    explicitly — it drives the assigned voice and is authoritative over
    call_insights.suggested_author."""
    ...
    return (
        'You are generating a short-form publish-ready social post from a '
        'single customer conversation. Use the source material below to '
        'ground the post in real customer language and concrete problem '
        'context.\n\n'
        '## Source conversation\n'
        ...
        '## Rationale (why this conversation merits a post)\n'
        f'{context.get("asset_rationale") or "(none)"}\n\n'
        '## Customer verbatim quotes\n'
        f'{verbatim_str}\n\n'
        '---\n'
        'Now write the post. Follow the format constraints from the base '
        'prompt (150-280 words, hook in the first line, no markdown headers, '
        'minimal or no bullet lists) and the voice characteristics from the '
        'author voice prompt. Output the post text only — no preamble, no '
        'quotation marks around the whole post, no "Here\'s the post:" prefix.'
    )
...
def call_claude(system_prompt, user_message):
    ...
    Raises:
      ...
      GenerationError   on other failures (skip draft)
...
# ─── orchestration ────────────────────────────────────────────────────
def process_one_draft(draft, dry_run):
    ...
    """
    draft_id   = draft['id']
    author_voice = draft['author_voice']
    insight_id = draft['call_insight_id']

    # 1. Pre-validate author_voice (pure local check, no DB). Terminal if
    #    invalid — no point claiming a row we cannot route to a prompt.
    if author_voice not in VALID_AUTHOR_VOICES:
        log('error', 'invalid_author_voice',
            draft_id=draft_id, author_voice=author_voice)
        if not dry_run:
            write_terminal_failure(draft_id, f'invalid_author_voice: {author_voice!r}')
        return 'failed'

    # 2. Claim the row.
    if not dry_run:
        try:
            if not claim_draft(draft_id):
                log('info', 'claim_lost', draft_id=draft_id)
                return 'lost_claim'
        except GenerationError as e:
            log('error', 'claim_error', draft_id=draft_id, detail=str(e)[:300])
            return 'failed'
    ...
    for attempt_idx, wait_s in enumerate(RETRY_BACKOFF_S):
        ...
        if wait_s > 0:
            log('info', 'retry_wait',
                draft_id=draft_id, attempt=attempt, wait_s=wait_s)
            time.sleep(wait_s)

        try:
            system_prompt = build_system_prompt(author_voice)
            context       = fetch_call_context(insight_id)
        except PromptNotFoundError as e:
            # Config issue — no amount of retry will help.
            log('error', 'prompt_not_found',
                draft_id=draft_id, author_voice=author_voice, detail=str(e)[:300])
            if not dry_run:
                write_terminal_failure(draft_id, f'prompt_not_found: {e}')
            return 'failed'
        except FetchError as e:
            last_error = f'fetch: {e}'
            log('warning', 'attempt_failed',
                draft_id=draft_id, attempt=attempt, stage='fetch',
                detail=str(e)[:300])
            if not dry_run:
                write_attempt_progress(draft_id, attempt, last_error)
            continue

        user_message = build_user_message(context, author_voice)
        log('info', 'calling_claude',
            draft_id=draft_id, author_voice=author_voice, attempt=attempt,
        ...
        try:
            ...
            draft_content = parse_draft(api_response)
            ...
            log('error', 'release_on_infra_failure',
                draft_id=draft_id, attempt=attempt, detail=str(e)[:300])
            if not dry_run:
                release_draft(draft_id, str(e))
            raise
        except GenerationError as e:
            last_error = f'generation: {e}'
            log('warning', 'attempt_failed',
                draft_id=draft_id, attempt=attempt, stage='generation',
                detail=str(e)[:300])
            if not dry_run:
                write_attempt_progress(draft_id, attempt, last_error)
            continue
        ...
        if dry_run:
            print(json.dumps({
                ...
                'attempt':         attempt,
                'metadata':        metadata,
                'draft_preview':   draft_content[:1200],
                'draft_length':    len(draft_content),
            }, indent=2, ensure_ascii=False), flush=True)
            return 'processed'

        write_success(draft_id, draft_content, prompt_used, metadata)
        log('info', 'draft_ready',
            draft_id=draft_id, author_voice=author_voice, attempt=attempt,
    ...
    log('error', 'all_attempts_failed',
        draft_id=draft_id, attempts=MAX_ATTEMPTS,
        detail=(last_error or 'unknown')[:300])
    if not dry_run:
        write_terminal_failure(draft_id, last_error or 'all_attempts_failed: unknown')
    return 'failed'
...
def main():
    ...
    # Pre-flight: verify the 'base' prompt is reachable in content_draft_prompts.
    # Asset-type prompts are loaded lazily per draft (each gets its own
    ...
    log('info', 'process_content_drafts_started',
        dry_run=args.dry_run,
        max_drafts=args.max_drafts,
        draft_id=args.draft_id,
        model=MODEL_ID,
        prompt_source='content_draft_prompts (DB)')

    fetch_limit = min(args.max_drafts, MAX_PER_RUN) if args.max_drafts else MAX_PER_RUN

    try:
        drafts = fetch_pending_drafts(fetch_limit, args.draft_id)
    except FetchError as e:
        log('error', 'fetch_drafts_failed', detail=str(e)[:500])
        return 1

    log('info', 'fetched_candidates', count=len(drafts))
    ...
    try:
        for i, draft in enumerate(drafts):
            result = process_one_draft(draft, args.dry_run)
    ...
    log('info', 'process_content_drafts_completed',
